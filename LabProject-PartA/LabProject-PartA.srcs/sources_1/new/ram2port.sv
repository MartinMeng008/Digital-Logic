...
module ram2port_module #(
   ...
   input wire [$clog2(Nloc)-1 : 0] addr1,     // Write Address, Address for specifying memory location
   input wire [$clog2(Nloc)-1 : 0] addr2,    // Read Address                                      //   num of bits in addr is ceiling(log2(number of locations))
   input wire [Dbits-1 : 0] din,             // Data for writing into memory (if wr==1)
   output wire [Dbits-1 : 0] dout1,
   output wire [Dbits-1 : 0] dout2            // Data read from memory (asynchronously, i.e., continuously)
   );
   ...
   assign dout2 = mem[addr2];                  // Memory read: read continuously, no clock involved
